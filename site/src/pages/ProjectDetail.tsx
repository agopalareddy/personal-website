 ...
 */
export function ProjectDetailPage({ p }: { p: ProjectEntry }) {
  return (
    <Layout
      activePage="projects"
      title={p.title}
    ...
    >
      ...
      <div className="card-actions" style={{ marginTop: "2rem", justifyContent: "flex-start", gap: "1rem" }}>
        {p.github && (
          <a href={p.github} target="_blank" rel="noopener" className="card-btn btn-github">
            <Icon name="MARK_GITHUB_16" /> GitHub <span className="sr-only">(opens in a new tab)</span>
          </a>
        )}
        {p.demo && (
