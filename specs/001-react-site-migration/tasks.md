...
- [x] T039 [P] [US3] ~~Implement `useCatalogFilter` + `CatalogFilter`~~ — built as `site/src/components/catalog/CatalogFilter.tsx` (static shell JSX, no hook) plus `site/src/islands/catalog-filter.ts` (one generic plain-DOM engine shared by both catalogs), same deviation as T040/T041: filter/search/sort/TOC is imperative DOM filtering over already server-rendered cards, not client-owned React state. Reuses the SSR'd `ProjectCard`/`ExperienceEntryCard` markup (tagged with `data-category`/`data-group`/`data-year`/`data-date`/`data-title`/`data-search`) instead of rebuilding cards from JSON like the legacy renderer did. All previously-skipped tests in `tests/e2e/experience-listing.spec.ts` un-skipped and passing.
...
- [x] T042 [US3] Wire `CatalogFilter` (T039) into `ProjectCatalogPage`/`ExperienceCatalogPage` as a hydrated island (T016's mechanism) — `catalog-filter` added to `vite.config.ts`'s rollup inputs and to `render.mjs`'s island list for both catalog pages.
...
- [x] T048 [US4] Run the complete `tests/a11y/` audit suite against every migrated page; confirm 0 new violations vs. the pre-migration baseline (FR-007, SC-003) — audited one exemplar per page template (both catalogs, one project detail, one experience detail per category, all 5 hand-authored pages): 0 violations everywhere except a pre-existing color-contrast issue inside the third-party Google Calendar `<iframe>` embed on `/availability/` (not our markup, not a regression).
- [x] T049 [US4] Run the complete `tests/e2e/` Playwright suite against the fully migrated site; confirm 100% pass (FR-008, SC-004) — 50/50 passed (desktop + mobile projects), 0 skipped.
- [x] T050 [US4] Diff the generated `sitemap.xml` against the pre-migration production sitemap; confirm no missing or stale entries (FR-009, SC-001, Acceptance Scenario 3) — 62/62 URLs identical, no missing or stale entries.
- [x] T051 [US4] Confirm the deployment step count/nature is unchanged (`git push` → `git fetch && git reset --hard` on the VM) (FR-010, SC-006) — unchanged; this migration never touched `AGENTS.md`'s Deploy section or `infra/nginx/`.
- [x] T052 [US4] Delete `scripts/generate_site.py` and `scripts/cv_parser.py` — only after T048–T051 all pass — done. `scripts/chrome.py` also deleted as a follow-on: it was imported only by `generate_site.py` and became fully orphaned once that was gone.
...
- [x] T053 [P] Update `AGENTS.md`'s "Editing experience or project content" / "Adding a new project (two catalogs!)" sections **and** `README.md`'s "Content Workflows" section (README.md:101,118,131 currently document the now-deleted `python3 scripts/generate_site.py --projects/--experiences` commands) — the dual-catalog problem they describe no longer exists post-migration (SC-002). Both updated to document `npm run build:site`; the "two catalogs" heading dropped since there's only one source of truth now. `CLAUDE.md` is a symlink to `AGENTS.md`, so it picked up the change automatically.
- [x] T054 [P] Audit every component under `site/src/components/` and `site/src/pages/` for raw hex values (must resolve to Primer functional tokens only, Constitution Principle III) and any unrequested decorative motion (Constitution Principle V) introduced during T017–T047 — clean. The only raw hex values present are in `Head.tsx`/`useTheme.ts` for browser metadata (`theme-color`/`mask-icon` meta tags), which the HTML spec requires as literal hex — not CSS, not a token violation. No `transition`/`animation`/`transform` found anywhere in `site/src`.
- [x] T055 [P] Run `openwiki --update "migrated site generation from Python generator to Vite+React SSG"` per the constitution's Documentation Sync workflow
- [x] T056 [P] Remove now-unused Python tooling references under `scripts/` (e.g. `.ruff_cache/`) once no Python remains there — `scripts/chrome.py` removed (see T052). `icons.py`, `update_nav.py`, `verify_ga4.py`, `experience_schema.md` remain: still imported/used independently of the deleted generator. `.ruff_cache/`/`__pycache__/` are gitignored local build artifacts, not tracked.
- [x] T057 Run the full `quickstart.md` validation pass end-to-end as the final confirmation — steps 1–6 all validated this session (clean `build:site`, 50/50 e2e, 0 a11y violations, 62/62 sitemap URL parity); the go/no-go gate for deleting the legacy pipeline is satisfied.
